...
describe('QualitySelector Component', () => {
  ...
  test('handles quality selection', () => {
    ...
    const lowButton = container.querySelector(
      '[data-testid="quality-low"]'
    ) as HTMLElement;
  ...
  test('shows selected quality as active', () => {
    ...
    let mediumButton = container.querySelector(
      '[data-testid="quality-medium"]'
    );
    expect(mediumButton).toHaveClass('selected');

    rerender(
      <QualitySelector value="high" onChange={mockOnChange} />
    );

    const highButton = container.querySelector(

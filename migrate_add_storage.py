...
if database_url:
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = f"sqlite:///{os.path.join(basedir, 'instance', 'barberx.db')}"
    is_postgres = False
...
def migrate():
    ...
    with app.app_context():
        try:
            ...
            print(f"Adding storage_used_mb column to users table ({'PostgreSQL' if is_postgres else 'SQLite'})...")
            
            # Add the column
            db.session.execute(db.text(
                "ALTER TABLE users ADD COLUMN storage_used_mb FLOAT DEFAULT 0.0"
            ))
            db.session.commit()

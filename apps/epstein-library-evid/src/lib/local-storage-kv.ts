 ...
 * 
 * Epstein Library — prefix: epstein-library:
 */
...
const KV_PREFIX = 'epstein-library:'
...
export async function getUser(): Promise<{ id: string; login: string }> {
  let userId = localStorage.getItem('epstein-library:user-id')
  if (!userId) {
    userId = `anon-${crypto.randomUUID?.() ?? Date.now()}`
    localStorage.setItem('epstein-library:user-id', userId)
  }
...
export async function llmStub(_prompt: string): Promise<string> {
  throw new Error(
